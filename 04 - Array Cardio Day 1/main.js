    ...
    const inventors = [
      ...
      let sortedInventors = inventors.sort((x, y) => x.year > y.year ? 1 : -1);
      console.log(sortedInventors);
  
      // Array.prototype.reduce()
      // 4. How many years did all the inventors live all together?
      let totalLivedYears = inventors.reduce((total, cur) => total + cur.passed - cur.year, 0);
      console.log(totalLivedYears);
  
      // 5. Sort the inventors by years lived
      let sortedInventorsByYearsLived = inventors.sort((x, y) => (y.passed - y.year) - (x.passed - x.year));
      console.log(sortedInventorsByYearsLived);
  
      // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
      // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
        // const links = document.querySelectorAll('.mw-category a');
        // let textInLinks = Array.from(links).map(link => link.textContent);
        // let filteredText = textInLinks.filter(text => text.includes('de'));
